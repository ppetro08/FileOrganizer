...
namespace FileOrganizer
{
    public class TreeViewModel : INotifyPropertyChanged
    {
        ...
        public TreeViewModel(string name, string fullPath, string extension)
        {
            Name = name;
            FullPath = fullPath;
            Extension = extension;
        ...
        public static void RenameItem(string oldFullPath, string newFullPath)
        {
            ...
            else
            {
                ...
                if (season != newSeason)
                {
                    ...
                    CheckTree(newSeason).Children.Add(new TreeViewModel(ns.File, newFullPath, ns.Extension));
                }
                else
                {
                    item.Name = ns.File;
        ...
        public static void AddItem(string fullPath)
        {
            if (HelperFunctions.IsMovie(fullPath))
            {
                ...
                MovieTree.Children.Add(new TreeViewModel(m.File, fullPath, m.Extension));
                MovieTree.Children = SortTree(MovieTree.Children);
            }
            else
            {
                var s = new Show(fullPath);
                var season = new TreeViewModel(s.Season);
                CheckTree(season).Children.Add(new TreeViewModel(s.File, fullPath, s.Extension));
                TvTree.Children = SortTree(TvTree.Children);
            }
        }
